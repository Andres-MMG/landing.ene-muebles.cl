...
const cardPath = join(process.cwd(), "apps/web/src/components/ProductCard.tsx");
...
describe("ProductCard — administrable product actions", () => {
  const product: Product = {
    id: 2,
    name: "Silla escolar 100%",
    slug: "silla-escolar",
  ...
  it("preserves exact action-label and message fallbacks when copy is absent", () => {
    const html = renderToStaticMarkup(
    ...
    expect(html).toContain(
      'href="https://wa.me/56912345678?text=Hola%2C%20me%20gustar%C3%ADa%20cotizar%20Silla%20escolar%20100%25%20para%20mi%20instituci%C3%B3n."',
    );
  });

  it("trims and renders action-label overrides with the custom product template", () => {
    const html = renderToStaticMarkup(
      createElement(ProductCard, {
        product,
        whatsappNumber: "+56912345678",
        actionCopy: {
          detailLabel: "  Revisar ficha CMS  ",
          whatsappLabel: "  Cotizar CMS  ",
          whatsappMessageTemplate: "Necesito {productName} & despacho.",
        },
      }),
    );

    expect(html).toContain("Revisar ficha CMS");
    expect(html).toContain("Cotizar CMS");
    expect(html).toContain(
      'href="https://wa.me/56912345678?text=Necesito%20Silla%20escolar%20100%25%20%26%20despacho."',
    );
  });

  it("uses exact label fallbacks for blank overrides", () => {
    const html = renderToStaticMarkup(
      createElement(ProductCard, {
        product,
        actionCopy: {
    ...
    expect(html).toContain("Ver detalle");
    expect(html).not.toContain("Cotizar CMS");
  });
});
