...
describe("buildProductJsonLd", () => {
  it("preserves verified core fields while omitting unverified inventory availability", () => {
  ...
  it("omits image when the product has no images", () => {
    const jsonLd = buildProductJsonLd({ ...baseProduct, images: [] }, "https://ene-muebles.cl");
...
describe("buildProductJsonLd offers gating (B2/U12)", () => {
  it("omits offers entirely when the price is zero (UI hides it)", () => {
    const jsonLd = buildProductJsonLd({ ...baseProduct, price: 0 }, "https://ene-muebles.cl");
  ...
  it("omits offers when price is negative or infinite", () => {
    expect(
      "offers" in buildProductJsonLd({ ...baseProduct, price: -1 }, "https://ene-muebles.cl"),
    ).toBe(false);
    expect(
      "offers" in
        buildProductJsonLd(
  ...
  it("defaults a missing currency to CLP like formatPrice", () => {
    const jsonLd = buildProductJsonLd({ ...baseProduct, currency: "" }, "https://ene-muebles.cl");
...
describe("hasVerifiedOffer", () => {
  it("accepts a finite positive CLP price", () => {
    ...
    expect(hasVerifiedOffer({ price: 89900, currency: undefined as unknown as string })).toBe(true);
