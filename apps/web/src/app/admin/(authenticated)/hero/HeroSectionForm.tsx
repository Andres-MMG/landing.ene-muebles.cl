 ...
 * inline (not extracted into shared lib) because each section form
 * is small and the duplication is cheaper than one more import
 * edge in the graph.
 */
...
/**
 * Build the JSON body for `PUT /api/admin/hero-section`. Required
 * fields are always sent. The secondary CTA pair follows the
 * "both must be cleared to remove" rule: if neither label nor href
 * is provided, the form signals "no secondary CTA" by sending both
 * as `null`, so Strapi wipes the saved pair. Sending only the label
 * without the href (or vice versa) is not allowed — the proxy route
 * rejects the body when only one half is sent.
 */
export function buildSubmitPayload(values: Values): Record<string, unknown> {
  ...
  if (secLabel === '' && secHref === '') {
    ...
    payload.secondaryCtaLabel = secLabel;
    payload.secondaryCtaHref = secHref;
  }
  return payload;
}

export function HeroSectionForm({ initial }: { initial: Values }) {
