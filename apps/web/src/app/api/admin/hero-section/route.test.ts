...
describe("PUT /api/admin/hero-section — validation", () => {
  ...
  it("forwards trimmed required fields and dropped optionals", async () => {
    ...
    expect(body.data?.imageCaption).toBe("Catálogo 2026");
    expect(body.data?.galleryCaption).toBe("Nuestras instalaciones");
    expect(body.data?.railSecondaryText).toBe("Fabricación y distribución");
  });
  ...
  it("forwards the upstream payload to the admin page with status 200", async () => {
    const upstream = {
      data: {
        ...
        imageCaption: "Catálogo institucional",
        galleryCaption: "Instalaciones",
        railSecondaryText: "Fabricación nacional",
      },
    };
    ...
    expect(body.data?.imageCaption).toBe("Catálogo institucional");
    expect(body.data?.galleryCaption).toBe("Instalaciones");
    expect(body.data?.railSecondaryText).toBe("Fabricación nacional");
  });
