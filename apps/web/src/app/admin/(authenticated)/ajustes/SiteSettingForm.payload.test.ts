...
describe("SiteSettingForm.buildSubmitPayload", () => {
  ...
  it("trims and includes configured commercial terms", () => {
    const payload = buildSubmitPayload(baseValues);
  ...
  it("sends null for blank commercial terms so saved values can be cleared", () => {
    const payload = buildSubmitPayload({
      ...baseValues,
      paymentTermsText: "",
      warrantyText: "   ",
    ...
    expect(payload).toMatchObject({
      ...
      warrantyText: null,
      quoteResponseTimeText: null,
    });
  });
  it("trims and includes all global navigation and product-action copy", () => {
    expect(buildSubmitPayload(baseValues)).toMatchObject({
      navigationHomeLabel: "Portada",
      navigationCatalogLabel: "Productos",
      navigationAboutLabel: "La empresa",
      navigationContactLabel: "Escríbenos",
  ...
  it("sends explicit null for every blank global navigation and product-action field", () => {
    const payload = buildSubmitPayload({
      ...baseValues,
      navigationHomeLabel: "",
    ...
    expect(payload).toMatchObject({
      ...
      navigationAboutLabel: null,
      navigationContactLabel: null,
      headerWhatsappLabel: null,
      mobileWhatsappLabel: null,
      whatsappProductMessageTemplate: null,
      productCardDetailLabel: null,
      productCardWhatsappLabel: null,
      productDetailWhatsappLabel: null,
      productDetailContactLabel: null,
    });
  });

  it("serializes foundedYear as an integer and blank as explicit null", () => {
    expect(buildSubmitPayload({ ...baseValues, foundedYear: " 1996 " }).foundedYear).toBe(1996);
    expect(buildSubmitPayload({ ...baseValues, foundedYear: "  " }).foundedYear).toBeNull();
  });
...
describe("SiteSettingForm.buildSubmitPayload — does NOT touch input state", () => {
  ...
  it("does not mutate commercial terms while serializing blank and nonblank values", () => {
    const values = {
      ...fullInputs,
      paymentTermsText: "  Pago contra orden de compra  ",
      warrantyText: "   ",
