...
type ProductGroup = { name: string; slug: string; items: Product[] };
...
function groupByCategory(products: Product[]): ProductGroup[] {
  ...
  for (const product of products) {
    ...
    const group = groups.get(slug) ?? { name, slug, items: [] };
...
function chunkIndexGroups(groups: ProductGroup[]): ProductGroup[][] {
  ...
  return Array.from(
    { length: Math.ceil(groups.length / CATEGORIES_PER_INDEX_PAGE) },
    (_, index) =>
      groups.slice(
        index * CATEGORIES_PER_INDEX_PAGE,
        (index + 1) * CATEGORIES_PER_INDEX_PAGE,
      ),
  );
}
...
function ProductPrintSlot({ product }: { product: Product }) {
  ...
  const material = product.materials?.filter(Boolean).join(", ") || product.observableMaterial;
  ...
  return (
    <article className="print-product-slot" data-product-slug={product.slug}>
      ...
      <div className="print-product-body">
        <p className="print-product-category">
          {product.productType || product.category?.name || "Catálogo"}
        </p>
        <h3>{product.name}</h3>
  ...
  return (
    ...
    >
      <header className="print-category-header">
        ...
        <p className="print-category-wordmark" aria-label="ENE-MUEBLES">
          ENE
        </p>
...
export default async function CatalogoImprimirPage() {
  ...
  const categoryPages = groups.flatMap((group) =>
    chunkProducts(group.items).map((products, index) => ({
      group,
      products,
      isFirstCategoryPage: index === 0,
    })),
  );
  ...
  return (
    <>
      ...
      >
        ...
        {categoryPages.map(({ group, products, isFirstCategoryPage }, index) => (
          <PrintCategoryPage
