...
const payload = {
  ...
  printCoverBody: "  Mobiliario para aulas, oficinas e instituciones.  ",
...
async function callPut(body: unknown) {
  ...
  const request = new Request("http://localhost/api/admin/catalog-page", {
    method: "PUT",
...
describe("GET /api/admin/catalog-page", () => {
  it("allows an active admin to read the draft singleton", async () => {
    ...
    expect(String((fetch as unknown as ReturnType<typeof vi.fn>).mock.calls[0]?.[0])).toBe(
      "http://localhost:1337/api/catalog-page?status=draft",
    );
  });
...
describe("PUT /api/admin/catalog-page", () => {
  it("allows an active admin to validate and forward the complete trimmed payload", async () => {
    ...
    expect(requireAdmin).toHaveBeenCalledOnce();
    expect(String((fetch as unknown as ReturnType<typeof vi.fn>).mock.calls[0]?.[0])).toBe(
      "http://localhost:1337/api/catalog-page?status=published",
    );
    ...
    expect(JSON.parse(String(init.body))).toEqual({
      data: {
        ...
        productCountSuffix: "productos certificados para instituciones.",
        documentationText:
          "Cada producto se entrega con ficha técnica y declaración de materiales.",
        printCtaLabel: "Imprimir PDF",
        printCoverTitle: "Mobiliario institucional",
        printCoverBody: "Mobiliario para aulas, oficinas e instituciones.",
        printIndexTitle: "Líneas de producto",
  ...
  it("rejects incomplete or blank content before contacting Strapi", async () => {
  ...
  it("invalidates only the catalog-page tag after a successful JSON write", async () => {
    mockFetch(200, { data: { ok: true } });
    await callPut(payload);

    const cache = await import("next/cache");
    const { STRAPI_CACHE_TAGS } = await import("@/lib/strapi");
    expect(cache.revalidateTag).toHaveBeenCalledWith(STRAPI_CACHE_TAGS.catalogPage, { expire: 0 });
  });

  it("does not invalidate the catalog-page tag after a failed write", async () => {
