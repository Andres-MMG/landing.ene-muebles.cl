...
describe("ProductForm.buildProductSubmitPayload — catalog-import fields", () => {
  ...
  it("clears every catalog-import field with null when blank", () => {
  ...
  it("clears catalogPage with null when zero or negative (would violate Strapi min: 1)", () => {
    expect(buildProductSubmitPayload({ ...baseValues, catalogPage: "0" }).catalogPage).toBeNull();
    expect(buildProductSubmitPayload({ ...baseValues, catalogPage: "-2" }).catalogPage).toBeNull();
  });

  it("clears catalogPage with null when not a finite number", () => {
    expect(
      buildProductSubmitPayload({ ...baseValues, catalogPage: "not-a-number" }).catalogPage,
    ).toBeNull();
  });
...
describe("ProductForm.emptyProductFormValues", () => {
  ...
  it("sends null for cleared optional fields and category so Strapi clears them", () => {
    const payload = buildProductSubmitPayload({
      ...emptyProductFormValues(),
      name: "Mesa escolar",
      description: "Mesa",
    ...
    expect(payload.productType).toBeNull();
    expect(payload.subcategory).toBeNull();
    expect(payload.usageEnvironment).toBeNull();
    expect(payload.observableColor).toBeNull();
    expect(payload.observableMaterial).toBeNull();
    expect(payload.catalogPage).toBeNull();
